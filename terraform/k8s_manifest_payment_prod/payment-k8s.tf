      ...
      template:
        ...
        spec:
          ...
          - name: emp-payment-service
            ...
            image: 453708032754.dkr.ecr.ap-south-1.amazonaws.com/prod-emp-payment-service:50c3e6220230914112741
            ports:
            - containerPort: 8090
            resources:
              limits:
...
resource "kubectl_manifest" "config_payment" {
  yaml_body = <<-YAML
    ...
    data:
      ...
      payment.payoneer.redirect-url: https://login.sandbox.payoneer.com/api/v2/oauth2/authorize?client_id=oTxFTNNC1WxeTmWv6AdKneG9Kp68bGDG&redirect_uri=https://rivexa.com/payoneer-integration&seller_id={sellerId}&scope=read%20write%20openid%20personal-details&response_type=code

      payment.redirection.url: https://rivexa.com/payoneer-integration 

      seller-service.name: emp-seller-service
      seller-service.url: http://emp-seller-service/emp-seller-service

      spring.datasource.url: jdbc:mysql://emp-prod-mysql-rds.c1ebqe1npo60.ap-south-1.rds.amazonaws.com:3306/payment?createDatabaseIfNotExist=true
      ...
      spring.redis.host: emp-redis-production.5wm9au.clustercfg.aps1.cache.amazonaws.com
