...
resource "aws_codebuild_project" "tna" {
  name          = "${var.project}-${var.tnaCodebuildName}-${var.env}"
  description   = "codebuild_project_for_tna_service"
  build_timeout = "50"
  service_role  = data.aws_iam_role.example1.arn

  source {
    type            = "CODECOMMIT"
    location        = "${var.sourceCodeUrl}${var.tna_repo_name}"
    git_clone_depth = 0
  ...
  logs_config {
    cloudwatch_logs {
      group_name  = "/aws/codebuild/${var.project}-${var.tnaCodebuildName}-${var.env}"
...
resource "aws_codepipeline" "tna_pipeline" {
  ...
  stage {
    ...
    action {
      ...
      configuration = {
        RepositoryName       = var.tna_repo_name
  ...
  stage {
    ...
    action {
      ...
      configuration = {
        ProjectName = aws_codebuild_project.tna.name
